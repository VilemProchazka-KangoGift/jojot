...
namespace JoJot.Services
{
    ...
    public static class DatabaseService
    {
        ...
        public static async Task OpenAsync(string dbPath)
        {
            ...
            await ExecuteRawPragmaAsync("PRAGMA journal_mode=WAL;");
            await ExecuteRawPragmaAsync("PRAGMA synchronous=NORMAL;");
            await ExecuteRawPragmaAsync("PRAGMA foreign_keys=ON;");
        ...
        /// Uses the write lock for consistency.
        /// </summary>
            ...
            try
            {
                ...
                await context.AppStates
                    ...
                    .ExecuteUpdateAsync(s => s
                        ...
                        .SetProperty(a => a.DesktopIndex, index));
                ...
                if (oldGuid != newGuid)
                {
                    await context.Notes
                        ...
                        .ExecuteUpdateAsync(s => s
                            .SetProperty(n => n.DesktopGuid, newGuid));
                }
            }
            catch (Exception ex)
            {
        ...
        /// Check-then-add (write lock prevents races) — replaces INSERT OR IGNORE.
        /// </summary>
        public static async Task CreateSessionAsync(string desktopGuid, string? desktopName, int? desktopIndex)
        {
            ...
            {
                using var context = CreateContext();
                bool exists = await context.AppStates.AnyAsync(a => a.DesktopGuid == desktopGuid);
                if (!exists)
                {
        ...
        public static async Task UpdateDesktopNameAsync(string desktopGuid, string newName)
        {
            ...
            try
            {
                ...
                await context.AppStates
                    ...
                    .ExecuteUpdateAsync(s => s
                        .SetProperty(a => a.DesktopName, newName));
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                await context.AppStates
                    ...
                    .ExecuteUpdateAsync(s => s
                        ...
                        .SetProperty(a => a.WindowState, geo.IsMaximized ? "Maximized" : "Normal"));
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (var (id, sortOrder) in updates)
                {
                    await context.Notes
                        .Where(n => n.Id == id)
                        .ExecuteUpdateAsync(s => s
                            .SetProperty(n => n.SortOrder, sortOrder));
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Returns tab previews (name + content excerpt) for recovery panel.
        /// </summary>
        ...
        public static async Task<int> GetNoteCountForDesktopAsync(string desktopGuid)
        {
            ...
            {
                using var context = CreateContext();
                return await context.Notes.CountAsync(n => n.DesktopGuid == desktopGuid);
            }
            catch (Exception ex)
            {
        ...
        public static async Task DeleteAllPendingMovesAsync()
        {
            ...
            {
                using var context = CreateContext();
                int deleted = await context.PendingMoves.ExecuteDeleteAsync();
        ...
        public static async Task MigrateNotesDesktopGuidAsync(string fromGuid, string toGuid)
        {
            ...
            try
            {
                ...
                int affected = await context.Notes
                    ...
                    .ExecuteUpdateAsync(s => s
                        .SetProperty(n => n.DesktopGuid, toGuid));
