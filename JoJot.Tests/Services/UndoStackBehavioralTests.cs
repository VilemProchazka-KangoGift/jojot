...
public class UndoStackBehavioralTests
{
    ...
    public void MaxTier1_ExactCount_NoEviction()
    {
        ...
        // Push exactly MaxTier1-1 more snapshots (total = MaxTier1 = 50)
        ...
        {
            var content = stack.Undo();
            content.Should().Be($"s{i - 1}");
        }
    }

    [Fact]
    public void MaxTier1_PlusOne_EvictsOldest()
    {
        var stack = CreateStack();
        stack.PushInitialContent("s0");

        // Push MaxTier1 more snapshots (total = MaxTier1 + 1 = 51, triggers eviction)
        for (int i = 1; i <= UndoStack.MaxTier1; i++)
        {
            stack.PushSnapshot($"s{i}");
        }

        // Undo all the way — s0 should be gone (evicted)
        string? last = null;
        while (stack.CanUndo)
        {
            last = stack.Undo();
        }
        last.Should().Be("s1"); // s0 was evicted
    }
    ...
    {
        var stack = CreateStack();
        // 😀 is U+1F600 — represented as 2 UTF-16 code units (surrogate pair)
        stack.PushInitialContent("😀");

        // "😀".Length = 2 (surrogate pair), so 2 * 2 = 4 bytes
    ...
    {
        var stack = CreateStack();
        // "A😀B" = 'A' (1) + surrogate pair (2) + 'B' (1) = 4 chars
        stack.PushInitialContent("A😀B");
    ...
    [Fact]
    public void UndoRedoCycle_100Pushes_NoCorruption()
    {
        var stack = CreateStack();
        stack.PushInitialContent("s0");

        // Push 100 snapshots (tier-1 will overflow and evict older ones)
        for (int i = 1; i <= 100; i++)
        {
            stack.PushSnapshot($"s{i}");
        }

        // Current should be s100
        ...
        // Due to MaxTier1=50, only ~50 entries remain in tier-1
        undoCount.Should().BeLessThanOrEqualTo(UndoStack.MaxTier1);
    ...
    public void PushSnapshot_TruncateAtFirstTier1Entry()
    {
        ...
        stack.Undo().Should().Be("n1");
        stack.Undo().Should().Be("s10");
        stack.Undo().Should().Be("s5");
        stack.Undo().Should().Be("s0");
    }
